...
%f1test
f1knnmodel = fitcknn(f1train,l1train,"NumNeighbors",1);
f1predictions = predict(f1knnmodel,f1test);
f1iscorrect = (f1predictions == l1test.COVID19);
f1accuracy = sum(f1iscorrect)/numel(f1predictions);

%f2test
f2knnmodel = fitcknn(f2train,l2train,"NumNeighbors",1);
f2predictions = predict(f2knnmodel,f2test);
f2iscorrect = (f2predictions == l2test.COVID19);
f2accuracy = sum(f2iscorrect)/numel(f2predictions);

%f3test
f3knnmodel = fitcknn(f3train,l3train,"NumNeighbors",1);
f3predictions = predict(f3knnmodel,f3test);
f3iscorrect = (f3predictions == l3test.COVID19);
f3accuracy = sum(f3iscorrect)/numel(f3predictions);

%f4test
f4knnmodel = fitcknn(f4train,l4train,"NumNeighbors",1);
f4predictions = predict(f4knnmodel,f4test);
f4iscorrect = (f4predictions == l4test.COVID19);
f4accuracy = sum(f4iscorrect)/numel(f4predictions);

%f5 test
f5knnmodel = fitcknn(f5train,l5train,"NumNeighbors",1);
f5predictions = predict(f5knnmodel,f5test);
...
%% ROC
[labels1, scores1] = resubPredict(f1knnmodel);
[X1,Y1,T1,AUC1] = perfcurve(f1knnmodel.Y,scores1(:,1),"0");
plot(X1,Y1)
title('ROC 1')
...
[labels2, scores2] = resubPredict(f2knnmodel);
[X2,Y2,T2,AUC2] = perfcurve(f2knnmodel.Y,scores2(:,1),"0");
plot(X2,Y2)
title('ROC 2')
xlabel('False Positive Rate')
ylabel('True Positive Rate')
...
[labels3, scores3] = resubPredict(f3knnmodel);
[X3,Y3,T3,AUC3] = perfcurve(f3knnmodel.Y,scores3(:,1),"0");
plot(X3,Y3)
title('ROC 3')
xlabel('False Positive Rate')
ylabel('True Positive Rate')
...
[labels4, scores4] = resubPredict(f4knnmodel);
[X4,Y4,T4,AUC4] = perfcurve(f4knnmodel.Y,scores4(:,1),"0");
plot(X4,Y4)
title('ROC 4')
xlabel('False Positive Rate')
ylabel('True Positive Rate')
...
[labels5, scores5] = resubPredict(f5knnmodel);
[X5,Y5,T5,AUC5] = perfcurve(f5knnmodel.Y,scores5(:,1),"0");
plot(X5,Y5)
title('ROC 5')
xlabel('False Positive Rate')
ylabel('True Positive Rate')
